...
std::string bufferClass::Serializestring(char mybuffer[])
{
	char serilizedbuffer[sizeof(mybuffer)];
	std::memcpy(serilizedbuffer,mybuffer,sizeof(mybuffer));
	return serilizedbuffer;
}

std::string bufferClass::deserealizestring(char myserilisedbuffer[])
{
	char deserilized[sizeof(myserilisedbuffer)];
	memcpy(deserilized, myserilisedbuffer, sizeof(myserilisedbuffer));
	return deserilized;
}